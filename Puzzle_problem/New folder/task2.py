...
def child(problem, action, parent):
    state = problem.result(action, parent.state)
    hr = problem.get_hr(state)
    return Node(state, parent.cost+1, hr, parent,action)

import heapq as hq
def informed_Search(problem):
    hr = problem.get_hr(problem.initial_state)
    current = Node(problem.initial_state,0, hr , None, None)
    frontier = []
